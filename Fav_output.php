...
<h1 align="left" style="margin-top: 20px; margin-left: 50px; margin-bottom: 0">Favourites</h1>
...
<?php
    ...
    $query = "SELECT count(*) as count FROM Favourite WHERE username = :username";
    $stmt = $conn->prepare($query);
    ...
    $sql = "SELECT titleid FROM Favourite WHERE username = :username";
    ...
    foreach ($favouriteTitles as $movie) {
        ...
        echo "<a href='Delete_from_Fav.php?id=$imdbId'><button style='width: 220px; background-color: #172c44'>Delete</button></a></div>";
