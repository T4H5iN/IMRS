...
try {
    $totalPages = ceil($conn->query("SELECT COUNT(*) FROM titles NATURAL JOIN rating NATURAL JOIN award WHERE type LIKE '%$type%'")->fetchColumn() / $entriesPerPage);

    $currentPage = $_GET['page'] ?? 1;

    $startIndex = ($currentPage - 1) * $entriesPerPage;

    $sql = "SELECT award.rank, titles.id, titles.type, primaryTitle, originalTitle, startYear, endYear, runtime, rating, noOfVotes FROM titles NATURAL JOIN rating NATURAL JOIN award WHERE type LIKE '%$type%' order by award.rank LIMIT $startIndex, $entriesPerPage";
